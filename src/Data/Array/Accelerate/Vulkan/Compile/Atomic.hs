...
import Data.Array.Accelerate.AST.Environment (Env (Push), prj')
import Data.Array.Accelerate.AST.Exp (PreOpenExp (..), PrimFun (..), expType)
import Data.Array.Accelerate.AST.Idx (Idx (..))
import Data.Array.Accelerate.AST.Operation (ArrayInstr (..), OpenExp)
...
import Data.Array.Accelerate.Vulkan.Compile.Var (bindNewVars, newVars)
...
-- Compile ops that can be emulated by compare-and-swap (CAS) lock-free loop
...
compileAtomicPrimFun (PrimAdd nt) env exp =
  case nt of
    ...
    _ -> Nothing
  where
    f = case extractAtomicArgs env exp of
      Nothing -> Nothing
  ...
  where
    ...
    f = case exp of
      (Pair (Evar (Var _ ZeroIdx)) (Evar (Var _ idx@(SuccIdx ZeroIdx)))) ->
        let (ExpString x) = prj' ZeroIdx env
         in let (ExpString y) = prj' idx env
             in Just $ "atomicAdd(" ++ x ++ ", -(" ++ y ++ "));\n"
      _ -> Nothing
