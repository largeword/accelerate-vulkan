...
import Prelude hiding (Ord (..), map, maximum, min, replicate, sum, zipWith, zipWith3)
...
step (T3 o m l) qb kb vb j =
  let Z_ ::. nbc ::. bc ::. d = shape kb
      ...
      lnew =
        zipWith5
          (\m_ mnew_ l_ mj_ lj_ -> exp (m_ - mnew_) * l_ + exp (mj_ - mnew_) * lj_)
          m
          mnew
          l
          mj
      ...
      o' =
        zipWith4
          (\l_ m_ mnew_ o_ -> l_ * exp (m_ - mnew_) * o_)
          (replicate (Any_ ::. d) l)
          (replicate (Any_ ::. d) m)
