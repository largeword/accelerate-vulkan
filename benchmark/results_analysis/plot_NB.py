...
def extract_result(file_path):
    ...
    pattern = re.compile(
        r'benchmarking \S+/Scalar Z \[(.+?)]\n.*?mean\s+(\d+\.\d+)\s*(ms|s|μs).*?\nstd dev\s+(\d+\.\d+)\s*(ms|s|μs)',
        re.DOTALL)
...
def plot_results(all_results):
    ...
    ax.set_xlabel('Scalar Z', fontsize=14)
    ax.margins(x=0)
    ...
    ax.set_title('Benchmarking of Naive N-Body Simulation', fontsize=16)
    ax.legend(title='Shade: std dev', loc='upper left')
    fig.tight_layout(pad=0)  # Remove extra padding

    svg_file = 'plot_NB.svg'
    pdf_file = 'plot_NB.pdf'
